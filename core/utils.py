...
def find_nearby_doctors(patient_lat, patient_lon, specialization=None, limit=5):
    ...
    if patient_lat and patient_lon:
        ...
        for doc in doctors:
            dist = dist_key(doc)
            doc.distance_km = dist if dist != 99999 else None
        doctors.sort(key=lambda d: d.distance_km if d.distance_km is not None else 99999)
    else:
...
def _fetch_osm_broadened(lat, lon, specialization='', city='', radius=20000):
    ...
    spec_lower = specialization.lower().strip() if specialization else ''
    if spec_lower.endswith('s') and spec_lower not in OSM_SPEC_MAP:
        singular = spec_lower[:-1]
        if singular in OSM_SPEC_MAP:
            spec_lower = singular

    osm_spec   = OSM_SPEC_MAP.get(spec_lower, '')
    headers = {'User-Agent': 'MediCateApp/1.0 (contact: dev@medicate.app)'}

    def run_query(q, timeout_val=20):
        try:
            resp = req.post(
    ...
    if osm_spec:
        # We use a fast timeout for regex-based specialization queries to avoid hanging if Overpass is slow.
        q1 = f'''
        [out:json][timeout:5];
        (
        ...
        out center body 15;
        '''
        elements = run_query(q1, timeout_val=8)
    ...
    out center body 25;
    '''
    elements = run_query(q2, timeout_val=18)
    results = _elements_to_results(elements, lat, lon)
    ...
    out center body 20;
    '''
    elements = run_query(q3, timeout_val=18)
    results = _elements_to_results(elements, lat, lon)
...
def _fetch_nominatim(specialization='', city=''):
    ...
    # Try specific queries first, then fall back to broad city hospitals/clinics
    queries_to_try = []
    if specialization:
        queries_to_try.append(f'{specialization} {city}')
        queries_to_try.append(f'{specialization} clinic {city}')
    queries_to_try.append(f'hospital {city}')
    queries_to_try.append(f'clinic {city}')
    queries_to_try.append(f'medical care {city}')
    ...
    for q in queries_to_try:
        try:
            ...
            logger.warning(f'Nominatim query for "{q}" failed: {e}')
